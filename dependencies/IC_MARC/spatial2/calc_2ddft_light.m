function feature_2ddfts = calc_2ddft_light(virtual_topography, virtual_chanlocs)
...
feature_2ddfts = zeros(1, size(virtual_topography, 1));

for i = 1:length(feature_2ddfts)
    % make pixelated map
    
    Th = cell2mat({virtual_chanlocs.theta});
    Th = pi/180*Th;
    Rd = cell2mat({virtual_chanlocs.radius});
    [x, y] = pol2cart(Th,Rd);
  ...
  plotrad = max(plotrad,0.5); 
    ...
    [Xi,Yi,Zi] = griddata(inty,intx,virtual_topography(i,intchans),yi',xi,'v4'); % interpolate data
    % pixelated map is now in Zi
    dim = length(virtual_topography);
