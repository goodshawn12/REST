...
opts = arg_define(0,varargin, ...
    ...
    arg({'reversed_datasets','DatasetProcessingOrder','DatasetsMachineOrder','ReversedDatasets'},0,[],'Sequence order for data sets. Can be used to run multiple invocations of bci_batchtrain in different orders of the same set, by passing in increasingly higher numbers (starting from 0) for subsequent machines.'), ...
    ...
    arg({'cache_loaded','CacheLoaded'},false,[],'Cache loaded results. If enabled, re-loading a result again will be faster.'), ...
    arg({'approach_restrict','ApproachRestrict'},{},[],'Restrict analysis to subset of approaches. Optional.','shapecheck',false), ...
    arg({'handler','ErrorHandler'},'auto',[],'Error handler to use. Function handle or name that takes a struct as generated by lasterror(). If this is set to auto, it will use hlp_handleerror if running locally and rethrow otherwise.'), ...
...
% if our setnames are paths, we eliminate the longest common initial subpath
if ~isempty(setnames) && any([setnames{:}] == '/' | [setnames{:}] == '\')
    for len = reverse(find(setnames{1} == '/'  | setnames{1} == '\'))-1
        good = true;
        for j=2:length(setnames)
            if ~strncmp(setnames{1},setnames{j},len)
                good = false;
                break;
            end
        end
        if good
            % found a subpath that matched: remove it
            setnames = cellfun(@(s){s((len+2):end)},setnames);
            break;
        end
    end
    ...
    for k=1:length(setnames)
        setnames{k}(setnames{k}=='/'|setnames{k}=='\') = '-'; end
end
...
% optionally restrict approaches to given set
if ~isempty(opts.approach_restrict)
    for fn=fieldnames(opts.approaches)'
        if ~any(strcmp(fn{1},opts.approach_restrict))
            opts.approaches = rmfield(opts.approaches,fn{1}); end
    end
end
...
if isempty(opts.datasets)
    disp('No datasets found.');
    return
end
if isempty(opts.approaches) || isempty(fieldnames(opts.approaches))
    disp('No approaches found.');
    return
end
...
[outputs,errors] = par_schedule(tasks, 'engine',opts.engine,'pool',opts.pool,'policy',opts.policy,'scope',opts.parallel_scope);
