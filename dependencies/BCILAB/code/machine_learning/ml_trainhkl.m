...
opts=arg_define([0 3],varargin, ...
    arg_norep('trials'), ...
    arg_norep('targets'), ...
    arg({'lambdas','Lambdas'}, 10.^(1:-.5:-8), [0 10^-10 10^2 Inf], 'Vector of regularization parameters to consider. If this is specified as a vector instead of a search() expression, HKL will internally optimize the parameter using a more efficient procedure than the framework''s search() facility.','cat','Core Parameters'), ...
...
if strcmp(loss,'logistic')
    if length(classes) > 2
        ...
        model = ml_trainvote(trials,targets,votingScheme,@ml_trainhkl,@ml_predicthkl,varargin{:});
        return;
