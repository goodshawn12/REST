...
arg_define([0 3],varargin, ...
    arg_norep('trials'), ...
    arg_norep('targets'), ...
    arg({'cost','Cost'}, search(2.^(-5:2:15)), [0 2^-7 2^15 Inf], 'Regularization parameter. Reasonable range: 2.^(-5:2:15), greater is stronger.'), ...
...
if strcmp(variant,'native')
    ...
    if length(classes) > 2
        ...
        model = ml_trainvote(double(trials),targets,votingScheme,@ml_trainsvm,@ml_predictsvm,varargin{:});
        return;
