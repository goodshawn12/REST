...
arg_define(varargin, ...
    ...
    arg({'zthresholds','PowerTolerances'}, [-3.5 5], [-Inf -10 15 Inf], 'Z score thresholds. The minimum and maximum standard deviations within which the power of a channel must lie (relative to a robust estimate of the clean EEG power distribution in the channel) for it to be considered "not bad".'), ...
    arg({'max_bad_channels','MaxBadChannels'}, 0.15, [0 Inf], 'Maximum fraction of bad channels. The maximum number or fraction of bad channels that a retained window may still contain (more than this and it is removed). Reasonable range is 0.05 (very clean output) to 0.3 (very lax cleaning of only coarse artifacts).'), ...
    arg({'window_len','WindowLength','window_length'}, 0.5, [0 0.25 5 Inf], 'Window length. Window length that is used to check the data for artifact content. This is ideally as long as the expected time scale of the artifacts but short enough to allow for several 1000 windows to compute statistics over.'), ...
    arg({'window_overlap','WindowOverlap'}, 0.66, [0 0 0.9 1], 'Window overlap fraction. The fraction of two successive windows that overlaps. Higher overlap ensures that fewer artifact portions are going to be missed (but is slower).'), ...
    arg_sub({'fit_params','ParameterFitting','parameter_fitting'}, {}, { ...
        arg({'max_dropout_fraction','MaxDropoutFraction'}, 0.1, [0 0 0.2 1], 'Maximum fraction that can have dropouts. This is the maximum fraction of time windows that may have arbitrarily low amplitude (e.g., due to the sensors being unplugged).'), ...
        arg({'min_clean_fraction','MinCleanFraction'}, 0.25, [0 0.1 0.5 1], 'Minimum fraction that needs to be clean. This is the minimum fraction of time windows that need to contain essentially uncontaminated EEG.'), ...
        arg({'truncate_quant','TruncateQuantile'}, [0.022 0.6], [0 0 0.8 1], 'Truncated Gaussian quantile. Quantile range [upper,lower] of the truncated Gaussian distribution that shall be fit to the EEG contents.','guru',true), ...
        arg({'step_sizes','StepSizes'}, [0.01 0.01], [0.00001 0.001 0.05 0.2], 'Grid search stepping. Step size of the grid search, in quantiles; separately for [lower,upper] edge of the truncated Gaussian. The lower edge has finer stepping because the clean data density is assumed to be lower there, so small changes in quantile amount to large changes in data space.','guru',true), ...
        arg({'shape_range','ShapeRange'}, 1.7:0.15:3.5, [0 1 8 Inf], 'Shape parameter range. Search range for the shape parameter of the generalized Gaussian distribution used to fit clean EEG.','guru',true), ...
