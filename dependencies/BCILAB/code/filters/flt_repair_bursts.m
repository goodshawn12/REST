...
arg_define(varargin, ...
    ...
    arg({'chunk_length','ChunkLength'},50000,uint32([0 0 100000 1000000000]), 'Maximum chunk length. Process the data in chunks of no larger than this (to avoid memory limitations).','guru',true), ...
...
[signal.data,state.asr] = asr_process(signal.data,signal.srate,state.asr,window_len,processing_delay,block_size,max_dimensions,chunk_length,use_gpu);
