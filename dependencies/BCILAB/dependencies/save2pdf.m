function save2pdf(outFileName,handle,nopainters,dpi)
%SAVE2PDF Saves a figure as a properly cropped pdf
%
%   save2pdf(outFileName,handle,dpi)
%
%   - outFileName: Destination to write the pdf to.
...
try
    outFileName = env_translatepath(outFileName);
catch
    disp('Could not call env_translatepath.');
end

try
    io_mkdirs(outFileName);
catch    
    disp('Could not call io_mkdirs.');
end
...
if ~nopainters
    set(handle, 'renderer', 'painter');
end

% Save the pdf (this is the same method used by "saveas")

if strfind(outFileName,'.pdf')
    print(handle,'-dpdf',outFileName,sprintf('-r%d',dpi))
elseif strfind(outFileName,'.eps')
    print(handle,'-depsc',outFileName,sprintf('-r%d',dpi))
elseif strfind(outFileName,'.png')
    print(handle,'-dpng',outFileName,sprintf('-r%d',dpi))
end
