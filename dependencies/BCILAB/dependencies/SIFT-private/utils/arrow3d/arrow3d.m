function varargout=arrow3d(start,stop,ang,ltype,p,n,ax)
%   Plot 3d Arrows
% 
...
if strcmp(ltype,'line')
    for i=1:length(dis)
        ...
        hlines(i)=line([start(i,1);start(i,1)+rx],[start(i,2);start(i,2)+ry],[start(i,3);start(i,3)+rz],'parent',ax);
    end
    hlgrd=[];
else
    for i=1:length(dis)
        ...
        hlines(i)=surf(ax,cx,cy,cz,'edgecolor','none','facecolor',[145,90,7]/255);
        hlgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),[145,90,7]/255,'parent',ax);
    end
end
...
for i=1:length(dis)
    ...
    [rx,ry,rz] = rotatedata(xi,yi,zi,nvec(i,:),cosrang(i),[0,0,0]);
    cx=start(i,1)+rx;cy=start(i,2)+ry;cz=start(i,3)+rz;
    hheads(i)=surf(ax,cx,cy,cz,'edgecolor','none','facecolor',[11 131 222]/255);
    %Draw the underside of taper
    hhgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),[7 86 145]/255,'parent',ax);
end
if SET_HOLD_OFF
    hold(ax,'off');
end
