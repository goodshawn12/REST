...
classdef PropertyGridField < hgsetget
    properties        
        ...
        Expert = false;     % Whether this property is flagged as "expert level"
    ...
    methods (Static)
        ...
        function fields = GenerateFromSpec(spec)
            global tracking;
            if ~isfield(tracking,'gui') || ~isfield(tracking.gui,'show_experimental')
                tracking.gui.show_experimental = true; end
            ...
            for f=1:length(spec)
                ...
                if strcmp(s.type,'logical') && iscell(s.range)
                    ...
                    elseif ~isempty(s.value)
                        error(['Only cell arrays supported for the value (here: ' hlp_tostring(s.value) ')']);
                    else
                        s.value = {};
                    end
                end
