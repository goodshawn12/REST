...
arg_define([0 1],varargin,...
    ...
    arg_subswitch({'adaptiveFF','AdaptiveFF'},'cooling',{ ...
        ...
        'adaptive', { ...
            arg({'decayRateAlpha','DecayRate'},0.02,[0 Inf],sprintf('Forgetting factor decay rate. \nThis applies to the learning rule: lambda = lambda - DecayRate*lambda + UpperBound*Gain*lambda^2.')), ...
            arg({'upperBoundBeta','UpperBound'},0.001,[0 Inf],sprintf('This applies to the learning rule: lambda = lambda - DecayRate*lambda + UpperBound*Gain*lambda^2.')), ...
            arg({'transBandWidthGamma','TransBandWidth'},1,[0 Inf],sprintf('This applies to the gain function: Gain(z) ~ tanh((z/z_{min} - TransBandCenter) / TransBandWidth), where z is the non-stationarity index.')), ...
            arg({'transBandCenter','TransBandCenter'},5,[1 Inf],sprintf('This applies to the gain function: Gain(z) ~ tanh((z/z_{min} - TransBandCenter) / TransBandWidth), where z is the non-stationarity index.')), ...
    ...
    arg_subtoggle({'evalConvergence','EvalConvergence'},[],...
        {...
            arg({'leakyAvgDelta','LeakyAvgNSI'},0.01,[0 1],sprintf('Leaky average value (delta) for computing non-stationarity index (NSI). NSI = norm(Rn), where Rn = (1-delta)*Rn + delta*(I-yf^T).')), ...
            arg({'leakyAvgDeltaVar','LeakyAvgVar'},1e-3,[0 1],sprintf('Leaky average value (delta) for computing variance of source activation. Var = (1-delta)*Var + delta*variance')), ...
