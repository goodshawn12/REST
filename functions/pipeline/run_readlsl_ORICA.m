function run_readlsl_ORICA(varargin)
...
%                               2012-03-21
    ...
    opts = arg_define(varargin, ...
        ...
        arg({'marker_query','MarkerStreamQuery','MarkerQuery'},[],[],'Marker stream query. Allows to select a marker stream to read from (e.g., by setting it to type=''Markers''). Leave it empty to ignore markers.','shape','row','type','char'), ...
    ...
    if ~isvarname(opts.new_stream)
        opts.new_stream = opts.new_stream(~ismember(opts.new_stream,['-' ' ']));
    end
