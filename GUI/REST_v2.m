function varargout = REST_v2(varargin)
% REST_v2 MATLAB code for REST_v2.fig
%
%      REST_v2(opts) starts the Real-time EEG Source-mapping Toolbox.
...
%           - playback (optional): a value of "true" will cause REST_v2 to
...
%             the rest_v2. A vector with two elements defines a start and stop
%             time (in seconds) to take from the data. This is usefule for
%             playback as you can have a large playback dataset and a small
%             calibration dataset.
%
%      REST_v2('Property','Value',...) creates a new REST_v2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before REST_v2_OpeningFcn gets called. An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to REST_v2_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @REST_v2_OpeningFcn, ...
    'gui_OutputFcn',  @REST_v2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before REST_v2 is made visible.
function REST_v2_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to REST_v2 (see VARARGIN)
...
% Choose default command line output for REST_v2
...
function varargout = REST_v2_OutputFcn(hObject, eventdata, handles)
...
onl_read_background(handles.streamName, @read_data, 20);
    ...
    function results = read_data()
        ...
        % taking only the last timestamp allows REST_v2 to run but the
...
% the window should be closed if REST_v2 is closed.
...
% Hints: contents = cellstr(get(hObject,'String')) returns popupmenuICclassifier contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popupmenuICclassifier
end
