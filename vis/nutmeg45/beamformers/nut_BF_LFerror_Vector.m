function out=nut_BF_LFerror_vector(Lp,data,flags);
%
% dipole source localization for the model y = F*s + v
% s~N(0,phi) , v~N(0,lam)
%
% INPUTS:
% Lp(nk,nf,nv) = lead field
% iryy(nk,nk) = inverse data covariance
% ires(nk,nk) = not used here, but following convention of other algos.
...
nf=size(Lp,2);
nv=size(Lp,3);
f = reshape(Lp,size(Lp,1),size(Lp,2)*size(Lp,3));
% clear Lp;
...
like=zeros(nem,1);
tphi=zeros(nem,1);
out.sbar0=single(zeros(nv,nt,nf));
out.sbar_out=single(zeros(nf,nt,nv));
% out.pmap=zeros(nv,nv);
% out.pmap0=zeros(nv,nv);
out.lmap0=zeros(nv,1);
out.lmap=zeros(nv,1);
out.lratmap=zeros(nv,1);
% out.pmapt0=zeros(nv,1);
% out.pmapt=zeros(nv,1);
out.W1=zeros(nk,nv*nf);
% 
...
%         phi_premult=inv(diag(sqrt(max(d-1,0))))*p'*sqrtm(lam);
% end
...
%     f1=squeeze(f(:,iv,:));
    ...
    switch flags.sbar_init
        ...
        case 'bf_nut'
            ...
            %             end
        case 'bf_ES'
            InvRzzLp = reshape(iryy*f,nk,nf,nv);
            InvESLp = reshape(ires*f,nk,nf,nv);
            weight=zeros(nk,nf,nv);
            for ii=1:nv
                invJ = Lp(:,:,ii)'*InvRzzLp(:,:,ii);
                J=double(nut_pinv(invJ));
                weight(:,:,ii)=InvESLp(:,:,ii)*J; % Nsensors*3 matrix
            end
            sbar(:,:,1) = transpose(squeeze(weight(:,1,:)))*y; %vox x time x orient
            sbar(:,:,2) = transpose(squeeze(weight(:,2,:)))*y;
            sbar(:,:,3) = transpose(squeeze(weight(:,3,:)))*y;
            %             phi=f1'*iryy*f1;
            %             w=nut_pinv(phi)*f1'*iryy;
            %             sbar=w*ESES*y;
        case 'algo1'
    ...
    out.sbar0=single(sbar);
    switch flags.phi_init
        case 'ssT'
            sresize=single(reshape(permute(sbar,[1 3 2]),nv*nf,nt));
            phi=sum((sresize.^2)');
        case 'magic_y'
...
%     out.pmap0(:,:,iv)=nut_pinv(phi);
    ...
    ryy=y*y';
%     nu=ones(nm,1);
clear weight InvESLp InvRzzLp sbar Lp
    % em iteration
