...
%    Rzz = bolts.params.Rzz1;
   ...
   weight=zeros(size(Lp,1),size(Lp,2),size(Lp,3));
   for i=1:size(Lp,3)
      ...
      weight(:,:,i)=InvRyyLp(:,:,i)*J; % Nsensors*2 matrix (alternative method)
      ...
      if(~mod(i,500))  % update progress every 500th iteration
             disp(['Please wait. Beamspace_noES has finished ' num2str(i) ' out of ' num2str(size(Lp,3)) ' voxels.']);
