...
%  Version 1.3.1nm  8 March 2011
%  Now takes gain into account when reporting orientations; NOTE this can break other
%  package that expect the previous odd behavior!

warning('This is readCTFds version 1.3.1nm -- an important polarity issue is corrected in this version, but some software like FieldTrip expects the previous uncorrected behavior!')
...
for kchan=1:res4.no_channels
  ...
  for qx=1:numCoils
    ...
    res4.senres(kchan).ori(:,qx)=buff(5:7) * -sign(res4.senres(kchan).gain);
