...
%
% INTERMEDIATE stuff
% y1 = y-f1*s, meaning all sensor data except for contributions from activity at given voxel, pointed by f1 = f(:,index,:);
% nf = number of lead field components
% s(nf,nt) = source factors
% x(nl,nt) = interference factors
% u(nm,nt) = background factors
...
sakeflags=flags.sake;

% nuts=rmfield(nuts,'Lp');

% t0=dsearchn(bolts.latency,0);
% y=double(bolts.meg(t0:end,:)');
% ypre=double(bolts.meg(1:t0-1,:)');
...
% ypre=yall(:,1:t0-1);
% y=yall(:,t0:sakeflags.timept(2));

yall=double(data.y');
yall=yall-mean(yall(:,sakeflags.timeptc(1):sakeflags.timeptc(2)),2)*ones(1,size(yall,2));
yall=yall/std(yall(:));
ypre=yall(:,sakeflags.timeptc(1):sakeflags.timeptc(2));
y=yall(:,sakeflags.timepta(1):sakeflags.timepta(2));
...
tphi=zeros(nem,1);
if nv*nt*nf*8<1e9
    sakeout.sbar0=zeros(nv,nt,nf);
    sakeout.sbar_out=zeros(nf,nt,nv);
    memsave=0;
else
    memsave=1;
end
...
switch sakeflags.sbar_init  %test this right away before start computing
    case 'bf_nut'
        if ischar(sakeflags.thisrun) %beamname
            load(sakeflags.thisrun);
        else %hack for simulations
            beam=run_bf_onsim2(sakeflags.thisrun);
        end
        sbar=zeros(nf,nt);
...
U(:,:)=U(:,iorder);
ES = U(:,1:sakeflags.nl);  % used to be .eigs when nl = eigs +1;
ESES=ES*ES';
...
for iv=1:nv
    if(~mod(iv,20))  % update progress every 20th voxel
        ...
        %             waitbar(iv/nv,h);
        %         end
        disp(['Please wait. SAKETINI has finished ' num2str(iv) ' out of ' num2str(nv) ' voxels.']);
    end
...
%             s=p0*diag(sqrt(d0))*p0';
    ...
    for iem=1:nem
        ...
        ryx=y*xbar';
        ryu=y*ubar';
        rss=sbar*sbar'+nt*igam(1:nf,1:nf);
        ...
        if (sakeflags.plot || iem==nem || iem==1)
            like(iem)=.5*nt*(ldlam+ldphi-ldgam)-.5*sum(sum(y.*(lam*y)))+.5*sum(sum(sxubar.*(gam*sxubar)))+.5*nk*(ldalp+ldpsi)+.5*nt*sum(log(nu));
            %             noise(iem)=.5*nt*log(det([a*rxx*a'+ b*ruu*b' + lam]));
            noise(iem)=.5*nt*(ldlam-ldgam)-.5*sum(sum(y.*(lam*y)))+.5*sum(sum(xubar.*(gamxu*xubar)))+.5*nk*(ldalp+ldpsi)+.5*nt*sum(log(nu));
        ...
        if sakeflags.updatelam
            ...
            lam=diag(nt./diag(ryy-2*fg*rys'-2*b*ryu'-a*ryx'+fg*rss*fg'+fg*rsx*a'+fg*rsu*b'+b*rsu'*fg'+b*rux*a'+b*ruu*b'));
        end
    ...
    sakeout.W1(:,:,iv)=W(1:nf,:)';
    %     Wout(:,:,iv)=W(1:nf,:);
    sakeout.pmap(:,:,iv)=nut_pinv(phi);
    if ~memsave
        sakeout.sbar_out(:,:,iv)=sbar;
    end
    
    sakeout.lmap0(iv)=like(1)/nt;
    sakeout.lmap(iv)=like(iem)/nt;
    sakeout.bic(iv)=-2*sakeout.lmap(iv)+nl*log(nt);
    sakeout.aic(iv)=-2*sakeout.lmap(iv)+nl*2;
    sakeout.lratmap(iv)=likerat(iem)/nt;
    sakeout.pmapt0(iv)=trace(sakeout.pmap0(:,:,iv));
    sakeout.pmapt(iv)=trace(sakeout.pmap(:,:,iv));
    if sakeflags.plot
        subplot(3,3,1);plot((1:iv)',pmapt0(1:iv));title('initial');ylabel('pmap')
        subplot(3,3,2);plot((1:iv)',pmapt(1:iv));title('final');
        subplot(3,3,4);plot((1:iv)',lmap0(1:iv));ylabel('lmap');xlabel('voxel');
        subplot(3,3,5);plot((1:iv)',lmap(1:iv));xlabel('voxel');
        subplot(3,3,9);plot((1:iv)',lratmap(1:iv));xlabel('voxel');
    end
end
% sakeout.W1=permute(Wout,[2 1 3]);
sakeout.sakeflags=sakeflags;
pack;
return
