function spli31(filtfile,wfile,chfile,ftype,varargin)
%SPLI31  Calculates source phase lag index for non-event-locked multi-trial data  
%        in which each trial is an artifact-free segment of a contiuous signal. 
%
%  spli31(filtfile,weightfile,connections,ftype,¦lofrq¦,¦hifrq¦,¦nfft¦,¦options¦)
...
%                   spli31(...,'roi',roifile), where roifile contains a structure 
%                   R which can be created with fcm_voxel2roi.
%               To use independent components:
%                   spli31(...,'ica',icadata), where icadata contains
...
if nargin<4
    fprintf('\nusage: spli31 filtfile wfile chfile ftype [lofrq] [hifrq] [nfft] [options]\n\n')
    return
end
...
if ischar(chfile) 
    if strcmpi(chfile,'all')
        ncomp=(numvirt*(numvirt-1))/2;
        ch=zeros(ncomp,2,'uint32');
        currstart=1;
        for cc=1:numvirt
            currlength=numvirt-cc;
            currend=currstart+currlength-1;
            ch(currstart:currend,:)=[[cc+1:numvirt]' cc*ones(currlength,1)];
            currstart=currend+1;
        end
        clear curr* 
    else
...
if ~isempty(varargin)
    ...
    ii=find(strcmpi('numtrial',varargin),1);
    if ~isempty(ii)
        numtrial=varargin{ii+1};
        meg.data=meg.data(:,:,1:numtrial);
        varargin(ii:ii+1)=[];
    end
...
% Info Display
fprintf('\nspli31 %s %s %s %s %3.1f %3.1f %d\n\n',filtfile,wfile,chfile,ftype,lofrq,hifrq,nfft)

% Prepare
fs = meg.srate;
tim = [meg.latency(1) meg.latency(end)];

all2all = ( ~isext && (ncomp>numtrial) );
if forcehilbert
    if all2all, numh=numvirt; else numh=numtrial; end
    if ~rem(lentrial,2)    % even 
        h    = 2*ones(lentrial/2+1,numh);
        h([1 end],:) = 1;
        neg0 = zeros(lentrial/2-1,numh);
        select = [1:lentrial/2+1];
    else    % odd 
        h    = 2*ones((lentrial+1)/2,numh);
        h(1,:) = 1;
        neg0 = zeros((lentrial-1)/2,numh);
        select = [1:(lentrial+1)/2];
    end
...
if all2all
    ...
    if ftype=='b'
        Coh = zeros(ncomp,1);
        perc10=floor(numtrial/10);
        if perc10>=1, numstep=10; step10=round(linspace(perc10,numtrial,numstep));
        ...
        switch bandmethod
            case 'hilbert'
                XY = XY ./ (numtrial*length(1:ds:lentrial));
            otherwise
                XY =XY ./ numtrial;
        end
        ...
        warning('Calculating PLI spectrogram with FFT is inofficial and highly experimental')
