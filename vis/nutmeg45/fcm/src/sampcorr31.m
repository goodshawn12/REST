function sampcorr31(filtfile,wfile,chfile,ftype,varargin)
%SAMPCORR31  Calculates source amplitude correlation for non-event-locked multi-trial data  
...
%            (or trial) and correlated between voxels. 
%
%  sampcorr31(filtfile,weightfile,connections,ftype,¦lofrq¦,¦hifrq¦,¦nfft¦,¦options¦)
...
%                   sampcorr31(...,'roi',roifile), where roifile contains a structure 
%                   R which can be created with fcm_voxel2roi.
%               To use independent components:
%                   sampcorr31(...,'ica',icadata), where icadata contains
...
%                   sampcorr31(...,'bandmethod',method), where method is either
...
if nargin<4
    fprintf('\nusage: sampcorr31 filtfile wfile chfile ftype [lofrq] [hifrq] [nfft] [options]\n\n')
    return
end
