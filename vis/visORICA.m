...
% Last Modified by GUIDE v2.5 17-Nov-2014 19:20:22
...
fhandle = figure('toolbar','none','Menubar','none','Name','IC Select','position',[1 1 990 990],'DeleteFcn',{@closeFigIC,hObject});
W = evalin('base','W');
...
for it = 1:handles.nic
    ...
    topoplotFast(W(:,it),handles.chanlocs);
    title(['IC' int2str(it)])
...
if get(button,'Value') % turned exclude on
    ...
    handles.exclude(handles.lock==ic) = [];
end
% save handles
guidata(hObject,handles);
% update ics to plot
